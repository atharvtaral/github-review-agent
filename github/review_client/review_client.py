...
class ReviewClient:
    ...
    def post_pr_comment(self, owner: str, repo: str, pull_number: int, comment_body: str) -> bool:
        """Post a general comment on the main PR conversation tab."""
        if not self.token:
            print("[ERROR] GITHUB_TOKEN is missing in .env file!")
            return False
        ...
        if response.status_code == 201:
            print("[SUCCESS] General review comment posted to GitHub PR successfully!")
            return True
        else:
            print(f"[ERROR] GitHub API Failed ({response.status_code}): {response.text}")
            return False

    def post_inline_comment(self, owner: str, repo: str, pull_number: int, commit_id: str, path: str, line: int, comment_body: str) -> bool:
        """Post an inline review comment on a specific line in the 'Files changed' tab."""
        if not self.token:
        ...
        if response.status_code == 201:
            print(f"[SUCCESS] Inline comment posted on {path} (line {line})!")
            return True
        else:
            print(f"[ERROR] Inline Comment Failed ({response.status_code}): {response.text}")
            return False